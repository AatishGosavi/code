...
export default function App() {
  ...
  // DYNAMIC DATE RANGE FILTER (replaces static "As of Date" indicator)
  ...
  // Handles the "Update" button click - triggers a query refresh against the selected date range
  const handleDateRangeUpdate = () => {
    triggerToast(`Refreshing "${activeView?.displayName || 'view'}" for ${fromDate} → ${toDate}...`);
    // In a real integration, this would re-fetch data from the backend, e.g.:
    // fetch(`/api/views/${activeView.title}?from=${fromDate}&to=${toDate}`)
    //   .then(res => res.json())
    //   .then(data => { /* update view data */ });
  };
  ...
  // Builds a CSV block (header + rows) for a single table, applying the active search filter
  const buildCSVBlock = (columns, data) => {
  ...
  // Exports the currently active SQL view (or all stacked tables, if consolidated) to a downloadable CSV file
  const handleExportCSV = () => {
    ...
    if (activeView.isConsolidated) {
      csvContent = activeView.tables
        .map(tbl => `# ${tbl.title}\n${buildCSVBlock(tbl.columns, tbl.data)}`)
        .join('\n\n');
    } else {
      csvContent = buildCSVBlock(activeView.columns, activeView.data);
    }
    ...
    link.href = url;
    link.download = `${activeView.title || 'sql_view_export'}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);

    triggerToast('View CSV data exported successfully');
  };
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-150 ${theme === 'dark' ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
      ...
      {/* LAYOUT STRUCTURE - always side-by-side: menu left, reports right */}
      <div className="flex flex-row items-stretch min-h-[calc(100vh-80px)]">
        
        {/* SIDEBAR NAVIGATION dropdowns */}
        <aside className={`w-72 shrink-0 border-r p-6 flex flex-col justify-between overflow-y-auto ${theme === 'dark' ? 'bg-slate-950' : 'bg-slate-50/50'}`}>
            ...
            <div className="flex flex-wrap gap-2.5 items-center">
              <div className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold ${theme === 'dark' ? 'bg-slate-800/80 text-slate-300' : 'bg-slate-100 text-slate-700'}`}>
                ...
                <span className="flex items-center gap-1.5">
                  <span className="hidden sm:inline text-[11px] uppercase tracking-wide text-slate-400">From</span>
                  <input
                    type="date"
                    value={fromDate}
                    onChange={(e) => setFromDate(e.target.value)}
                    className={`bg-transparent border rounded-md px-1.5 py-0.5 text-xs font-semibold outline-none focus:ring-1 focus:ring-emerald-500 ${
                      theme === 'dark' ? 'border-slate-700 text-slate-200' : 'border-slate-300 text-slate-700'
                    }`}
                  />
                  <span className="hidden sm:inline text-[11px] uppercase tracking-wide text-slate-400">To</span>
                  <input
                    type="date"
                    value={toDate}
                ...
                <button
                  ...
                  className="ml-1 flex items-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white px-2.5 py-1 rounded-md text-[11px] font-bold active:scale-95 transition-transform"
                >
